...
df_vis_basis = df_complete.loc[:, rename_dict.keys()].rename(columns=rename_dict).assign(
    brutto_plus_alg = lambda x: x["bruttolohn"] + x["arbeitslosen_geld"],
    netto_plus_alg = lambda x: x["nettolohn"] + x["arbeitslosen_geld"],
)
...
df_vis_diff = (
    ...
    .assign(
        ...
        Bruttomonatslohn                           = lambda x:  x.bruttolohn,
        Arbeitslosen_Geld_Reduktion                = lambda x: (-x.diff__arbeitslosen_geld).clip(lower=0),
        # Nettolohn_minus_Arbeitslosengeld_Reduktion_true = lambda x: x.diff__nettolohn - x.Arbeitslosen_Geld_Reduktion,
        Nettolohn_minus_Arbeitslosengeld_Reduktion = lambda x:  abs(x.diff__nettolohn - x.Arbeitslosen_Geld_Reduktion),
...
df_vis_diff_percent = (
    ...
    .loc[:,
       ["Kindergeld","Nettolohn_minus_Arbeitslosengeld_Reduktion","Arbeitslosen_Geld_Reduktion","Lohnsteuer","Kirchensteuer","Solidaritätszuschlag","Rentenversicherung","Arbeitslosenversicherung","Krankenversicherung","Pflegeversicherung","Arbeitslosen_Geld_Zuwachs"
    ]]
)
...
df_vis_diff_percent.plot.area(ax=axes[1],
    ...
    color={
        ...
        "Kindergeld": "#449A90",
        "Nettolohn_minus_Arbeitslosengeld_Reduktion": "#63BE7B",
...
fig.savefig("result_family_detailed.png", format="png")
